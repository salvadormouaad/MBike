...
export default function BikesSlider() {
    ...
    const allBikes = products
        .filter((product) => Number(product.categoryId) === 1)
        .map((product) => ({
    ...
    if (allBikes.length === 0) return <p className="text-center">No bikes available.</p>;
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-16">
                    ...
                    <p className="text-lg md:text-xl text-gray-600 max-w-3xl mx-auto">
                        Explore our range of high-quality cycling Bikes
                    </p>
                </div>
                        ...
                        {allBikes.map((bike) => (
                            <SwiperSlide key={bike.Id}>
                                <div className="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-transform duration-300">
                                    <img
                                        src={backendBaseUrl + bike.Images}
                                        alt={bike.Name}
                                        className="w-full h-72 object-contain rounded-lg"
                                        loading="lazy"
                                    />
                                    <h3 className="text-lg font-semibold text-gray-900 mt-4">{bike.Name}</h3>
                                    <p className="text-gray-600 text-sm">Weight: {bike.Poids} | Price: {bike.Price}</p>
                                    <button
