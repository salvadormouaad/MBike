...
function Section_Helmets() {
    ...
    // Filter for helmets (categoryId: 2 for "Helmets")
    const helmets = products
        .filter((product) => product.categoryId === 2) // categoryId: 2 is for Helmets
        .map((product) => ({
            ...
            <div className="text-center my-16">
                <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold text-gray-900 mb-4">
                    <span className="bg-clip-text text-transparent bg-gradient-to-r from-gray-800 to-gray-600">
                        Explore Our Geometriques Zelige Collection
                    </span>
                </h2>
    ...
    if (helmets.length === 0) {
        return (
            <div className="text-center my-16">
                ...
                <p>No Zelige available at the moment.</p>
            </div>
        );
    }

    // Display only the first helmet item
    const val = helmets[0];
...
export default Section_Helmets;
