...
export default function Carde5() {
    const reduxGear = useSelector((state) => {
        return state.gear.vetements.homme.pantalon_bretelles;
    });
    return (
        <>
            {/*
            ...
            {reduxGear.map((val, index) => {
                return (
                    index === 0 && (
                        <div
                            key={index}
                            className="relative  w-[350px] bg-white rounded-3xl shadow-md overflow-hidden group hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-200"
                        >
                            <div className="relative h-56 sm:h-64 md:h-72 overflow-hidden">
                                <img
                                    src={val.images[0]}
                                    alt={`Pantalon ${val.id}`}
                                    className="w-70 h-full object-cover transition-transform duration-500 group-hover:scale-105 grayscale group-hover:grayscale-0"
                                    loading="lazy"
                                />
